...
def main():
    ...
    exp_path_root_config = OmegaConf.load("./configs/pnp/setup-test.yaml")
    ...
    def save_sampled_img(x, i, save_paths):
        x_samples_ddim = pnp_model.decode_first_stage(x[0].unsqueeze(0))
        ...
        img.save(os.path.join(save_paths, f"{i}.png"))

    def ddim_sampler_callback(pred_x0, xt, i):
        save_sampled_img(pred_x0, i, predicted_samples_paths)

    def load_target_features(index):
        ...
        source_experiment_out_layers_path = os.path.join(exp_path_root, exp_config.source_experiment_name, f"{index}", "feature_maps")
        source_experiment_qkv_path = os.path.join(exp_path_root, exp_config.source_experiment_name, f"{index}", "feature_maps")
    ...
    for folder in range(1, 9):
        # 根据文件夹名生成图片路径
        input_image_path = f"tests/test/{folder}/samples/0.jpg"
        detected_map_path = f"tests/test/{folder}/detected_map.png"
        outpath = f"tests/test/{folder}/conpnp_translations"
        out_label = f"{folder}_translated"
        predicted_samples_paths = os.path.join(outpath, f"predicted_samples_{out_label}")
        os.makedirs(outpath, exist_ok=True)
        os.makedirs(predicted_samples_paths, exist_ok=True)
        # save args in experiment dir
        with open(os.path.join(outpath, "args.json"), "w") as f:
            json.dump(OmegaConf.to_container(exp_config), f)

        injected_features = load_target_features(folder)
        ...
        with torch.no_grad():
            with precision_scope("cuda"):
                ...
                x_samples_ddim = pnp_model.decode_first_stage(samples_ddim)
                x_samples_ddim = torch.clamp((x_samples_ddim + 1.0) / 2.0, min=0.0, max=1.0)
                x_samples_ddim = x_samples_ddim.cpu().permute(0, 2, 3, 1).numpy()
                if opt.check_safety:
                ...
                sample_idx = 0
                for _ , x_sample in enumerate(x_image_torch):
                    x_sample = 255. * rearrange(x_sample.cpu().numpy(), 'c h w -> h w c')
                    img = Image.fromarray(x_sample.astype(np.uint8))
                    # 在保存图片时，将文件夹名添加到文件名中
                    img.save(os.path.join(outpath, f"{out_label}.png"))
                    sample_idx += 1

        print(f"Controled_PnP results saved in: {outpath}")

...
os.environ['TORCH_HOME']='/data/kongweili/StyleID-main/pretrained_pth'
...
def main():
    ...
    parser.add_argument('--output_path', type=str, default='output/batch_20')
    ...
    for folder in range(1, 11):
        sty_name = f"tests/batch_20/{folder}/pnp_translations/{folder}_translated.jpg"
        cnt_name = f"tests/batch_20/{folder}/samples/0.jpg"
        # sty_img_list = sorted(os.listdir(opt.sty))
        # cnt_img_list = sorted(os.listdir(opt.cnt))

        begin = time.time()
        init_sty = load_img(sty_name).to(device)
        seed = -1
        ...
        init_cnt = load_img(cnt_name).to(device)
        ...
        with torch.no_grad():
            with precision_scope("cuda"):
                with model.ema_scope():
                    # inversion
                    output_name = f"{os.path.basename(cnt_name).split('.')[0]}_stylized_{os.path.basename(sty_name).split('.')[0]}.jpg"

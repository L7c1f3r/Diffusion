...
from features_extraction_for_test import load_model_from_config
# os.environ['CUDA_VISIBLE_DEVICES'] = '0, 1, 2, 3, 4, 5'
...
def main():
    ...
    def save_sampled_img(x, i, save_paths):
        x_samples_ddim = model.decode_first_stage(x[0].unsqueeze(0))
        ...
        img.save(os.path.join(save_paths, f"{i}.jpg"))
    ...
    for folder in range(1, 11):
        outpath = os.path.join(exp_path_root, exp_config.source_experiment_name, f"{folder}/pnp_translations")
            ...
            with precision_scope("cuda"):
                with model.ema_scope():
                    ...
                    for k, x_sample in enumerate(x_image_torch):
                        x_sample = 255. * rearrange(x_sample.cpu().numpy(), 'c h w -> h w c')
                        img = Image.fromarray(x_sample.astype(np.uint8))
                        img.save(os.path.join(outpath, f"{out_label}.jpg"))
                        sample_idx += 1

        print(f"PnP results saved in: {outpath}")

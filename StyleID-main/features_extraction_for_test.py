...
from ldm_original.models.diffusion.ddim_original import DDIMSampler
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="?",
        default="configs/pnp/feature-extraction-real.yaml",
    ...
    setup_config = OmegaConf.load("./configs/pnp/setup-test.yaml")
    ...
    save_feature_timesteps = exp_config.config.ddim_steps

    # 获取图片文件夹中的所有图片
    image_files = glob.glob('./dataset/batch_20/*.jpg')
    ...
    for idx, image_file in enumerate(image_files, start=1):
        # 为每一张图片创建一个新的文件夹
        outpath = f"{exp_path_root}/{exp_config.config.experiment_name}/{idx}"
        print(outpath)
        ...
        # 使用图片的路径来加载图片
        init_image = load_img(image_file).to(device)
            ...
            with precision_scope("cuda"):
                with model.ema_scope():
                    ...
                    for x_sample in x_image_torch:
                        x_sample = 255. * rearrange(x_sample.cpu().numpy(), 'c h w -> h w c')
                        img = Image.fromarray(x_sample.astype(np.uint8))
                        img.save(os.path.join(sample_path, f"{sample_idx}.jpg"))
                        sample_idx += 1

        print(f"Sampled images and extracted features saved in: {exp_path_root}/{exp_config.config.experiment_name}/{idx}")
        # break
